...
def test_get_course_by_id_exists():
    response = client.get("/courses/6585137b68d9683539c39ff4")
    assert response.status_code == 200
    course = response.json()
    # get the course from the database
    course_db = db.courses.find_one({'_id': ObjectId('6585137b68d9683539c39ff4')})
...
def test_get_chapter_info():
    response = client.get("/courses/6585137b68d9683539c39ff4/1")
    assert response.status_code == 200
    chapter = response.json()
    assert chapter['name'] == 'Image Classification'
    assert chapter['text'] == 'Computer Vision Course'
    
    
def test_get_chapter_info_not_exists():
    response = client.get("/courses/6585137b68d9683539c39ff4/990")
    assert response.status_code == 404
    assert response.json() == {'detail': 'Chapter not found'}


def test_rate_chapter():
    course_id = "6585137b68d9683539c39ff4"
    chapter_id = "1"
    rating = 1
